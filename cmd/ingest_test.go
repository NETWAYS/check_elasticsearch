	...
	tests := []IngestTest{
		{
			...
			expected: "[OK] - Ingest operations alright \n \\_[OK] Number of failed ingest operations for mypipeline: 5 | pipelines.mypipeline.failed=5c;10;20 pipelines.mypipeline.count=10c pipelines.mypipeline.current=3\n",
		},
		{
			...
			expected: "[OK] - Ingest operations alright \n \\_[OK] Number of failed ingest operations for foobar: 5 | pipelines.foobar.failed=5c;10;20 pipelines.foobar.count=10c pipelines.foobar.current=3\n",
		},
		{
			name: "ingest-warn",
			server: httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			expected: "[WARNING] - Ingest operations may not be alright \n \\_[WARNING] Number of failed ingest operations for mypipeline: 5 | pipelines.mypipeline.failed=5c;3;20 pipelines.mypipeline.count=10c pipelines.mypipeline.current=3\nexit status 1\n",
		},
		{
			name: "ingest-crit",
			server: httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			expected: "[CRITICAL] - Ingest operations not alright \n \\_[CRITICAL] Number of failed ingest operations for mypipeline: 5 | pipelines.mypipeline.failed=5c;10;3 pipelines.mypipeline.count=10c pipelines.mypipeline.current=3\nexit status 2\n",
		},
		{
			name: "ingest-invalid",
			server: httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
